...
// -------------------- API Abstraction --------------------
class Api {
  static REST_URL = "https://api/students"; // change to your REST API base

  static async list() {
    if (!API_MODE) return Storage.list();
    const r = await fetch(this.REST_URL);
    return await r.json();
  }
  ...
  static async update(id, student) {
    ...
    const r = await fetch(`${this.REST_URL}/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(student),
    });
...
class UI {
  ...
  async init() {
    ...
    this.elements.apiToggle.checked = API_MODE;
    this.elements.apiToggle.addEventListener("change", (e) => {
      toast("Toggle API mode locally — restart needed to switch API calls.");
    });
    this.elements.search.addEventListener(
      "input",
      debounce((e) => this.onSearch(e), 250)
    );
    this.elements.sortBy.addEventListener("change", (e) => {
      this.store.sort = e.target.value;
      this.store.apply();
      this.render();
    });
    this.elements.filterMajor.addEventListener("change", (e) => {
      this.store.filterMajor = e.target.value;
      this.store.apply();
      this.render();
    });
    this.elements.prevPage.addEventListener("click", () => {
      if (this.store.page > 1) this.store.page--;
      this.render();
    });
    ...
    this.elements.pageSize.addEventListener("change", (e) => {
      this.store.pageSize = Number(e.target.value);
      this.store.page = 1;
      this.render();
    });
    ...
    $$(".sidebar nav a").forEach((a) =>
      a.addEventListener("click", (e) => {
        ...
        const s = "section-" + e.currentTarget.dataset.section;
        $$(".section").forEach((sec) =>
          sec.id === s
  ...
  populateFilter() {
    const majors = [
      ...new Set(this.store.items.map((s) => s.major).filter(Boolean)),
    ];
    const sel = this.elements.filterMajor;
    sel.innerHTML =
  ...
  render() {
    ...
    rows.forEach((s, i) => {
      ...
      tr.innerHTML = `
        <td><input class="row-check" data-id="${s.id}" type="checkbox" ${
        this.store.selection.has(s.id) ? "checked" : ""
      }></td>
        ...
        <td>${s.major || ""}</td>
        <td class="actions">
          <button class="btn small" data-action="edit" data-id="${
            s.id
          }">Edit</button>
          <button class="btn small outline" data-action="clone" data-id="${
            s.id
          }">Clone</button>
    ...
    $$(".row-check").forEach((cb) =>
      cb.addEventListener("change", (e) => {
        ...
        if (e.currentTarget.checked) this.store.selection.add(id);
        else this.store.selection.delete(id);
      })
    );
    $$(".actions button").forEach((btn) =>
      btn.addEventListener("click", (e) => {
        ...
        if (act === "edit")
          this.openForm(this.store.items.find((x) => x.id === id));
    ...
    const totalPages = Math.max(
      1,
      Math.ceil(this.store.filtered.length / this.store.pageSize)
    );
  ...
  openForm(student = null) {
    ...
    modal.innerHTML = `
      <div class="card">
        ...
        <div class="form-row">
          <input id="fName" placeholder="Full name" value="${
            student ? student.name : ""
          }">
          <input id="fEmail" placeholder="Email" value="${
            student ? student.email : ""
          }">
        </div>
        <div style="height:10px"></div>
        <div class="form-row">
          <input id="fMajor" placeholder="Major" value="${
    ...
    $("#save").addEventListener("click", async () => {
      ...
      if (student) {
        ...
        // update local list
        this.store.items = this.store.items.map((x) =>
          x.id === student.id ? { ...x, ...payload } : x
        );
        toast("Updated");
      } else {
  ...
  async undo() {
    const last = this.store.undoStack.pop();
    if (!last) return toast("Nothing to undo");
    if (last.op === "delete") {
      const it = last.item;
      this.store.items.push(it);
      Storage.save(this.store.items);
      this.store.apply();
      this.render();
      toast("Undo delete");
    }
    if (last.op === "bulkDelete") {
      this.store.items.push(...last.items);
      Storage.save(this.store.items);
      this.store.apply();
      this.render();
  ...
  clone(id) {
    const s = this.store.items.find((x) => x.id === id);
    const copy = { ...s, id: uid(), name: s.name + " (copy)" };
    this.store.items.push(copy);
    Storage.save(this.store.items);
    this.store.apply();
    this.render();
    toast("Cloned");
  ...
  toggleSelectAll(value) {
    $$(".row-check").forEach((cb) => {
      ...
      if (value) this.store.selection.add(id);
      else this.store.selection.delete(id);
    });
  }
  ...
  onSearch(e) {
    ...
    this.store.filtered = this.store.items.filter(
      (s) =>
  ...
  async handleImport(e) {
    ...
    const created = arr.map((r) => {
      const obj = {
        name: r.name || r.FullName || r.fullname || r.Name || "",
        email: r.email || r.Email || "",
        major: r.major || r.Major || "",
      };
      return Storage.create(obj);
    });
    this.store.items = Storage.list();
    this.store.apply();
    this.render();
    toast("Imported " + created.length + " rows");
  }
  exportCSV() {
    const data = this.store.items.map(({ id, ...rest }) => rest);
  ...
  initChart() {
    ...
    this.chart = new Chart(ctx, {
      ...
      data: {
        labels: [],
        datasets: [{ label: "Students by Major", data: [] }],
      },
